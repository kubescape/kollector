...
docker build --no-cache -f Dockerfile.test -t quay.io/armosec/k8s-ca-dashboard-aggregator:$WTAG .
...
kubectl set image deployment/ca-dashboard-aggregator -n cyberarmor-system ca-aggregator=quay.io/armosec/k8s-ca-dashboard-aggregator:$WTAG || true
